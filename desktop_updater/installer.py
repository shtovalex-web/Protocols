...
from desktop_updater.windows_app_bundle import (
...
def stage_payload_copy(
    ...
    *,
    ...
    if actual_size != expected_size:
        hint = get_config().publish_script_hint
        msg = (
            f"Размер файла обновления не совпадает с manifest.json: "
            f"{actual_size} != {expected_size}. "
            f"Пересоберите и опубликуйте обновление: {hint}"
        )
        raise UpdateInstallerError(msg)
...
def restart_cmd_path(exe_path: Path) -> Path:
    return exe_path.resolve().parent / get_config().resolved_restart_cmd_name
...
def _write_restart_cmd(exe_path: Path, *, parent_pid: int) -> Path:
    ...
    pid = int(parent_pid)
    zip_name = cfg.resolved_app_bundle_zip_name
    exe_file = cfg.exe_name
    content = (
        "@echo off\r\n"
        ...
        ")\r\n"
        ":start_app\r\n"
        f'if exist "%~dp0{APP_UPDATE_STAGING_DIR}\\{exe_file}" (\r\n'
        ...
        ")\r\n"
        f'if exist "%~dp0{zip_name}.new" del /q "%~dp0{zip_name}.new"\r\n'
...
def cleanup_restart_cmd(exe_path: Path) -> bool:
    """Удалить restart.cmd после перезапуска."""
