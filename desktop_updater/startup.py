# -*- coding: utf-8
"""Проверка обновлений при старте (замена .exe через rename)."""
...
from desktop_updater.client_config import load_update_config, resolve_update_share_root
from desktop_updater.data_installer import apply_data_updates, mark_data_version_installed
from desktop_updater.info import data_dir_for_exe, installed_version_from_data
from desktop_updater.installer import (
...
from desktop_updater.manifest import UpdateManifestError
from desktop_updater.pending_changelog import write_pending_changelog
from desktop_updater.registry import get_config
from desktop_updater.scan import resolve_latest_update
from desktop_updater.success import notify_update_success_and_exit
from desktop_updater.windows_app_bundle import is_windows_app_bundle
...
def should_check_for_updates(*, force: bool = False) -> bool:
    ...
    if force:
        return True
    if os.environ.get(cfg.env_force_check) == "1":
        return True
    return is_frozen()
...
def app_version() -> str:
    ...
    if exe is not None:
        ...
        if data_version:
            return data_version
    return (get_config().app_version or "").strip()
