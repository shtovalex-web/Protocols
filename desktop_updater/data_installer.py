...
from desktop_updater.info import data_dir_for_exe, write_update_info
from desktop_updater.installer import UpdateInstallerError, stage_payload_copy
from desktop_updater.manifest import DataFilePayload, UpdateManifest
...
def _permission_error_hint(path: Path) -> UpdateInstallerError:
    ...
    if name.endswith(".docx"):
        hint = "Закройте документы Word (шаблоны, инструкции) и повторите обновление."
    else:
...
def apply_data_updates(
    ...
    prefix = get_config().data_update_temp_prefix
    with tempfile.TemporaryDirectory(prefix=prefix) as temp_raw:
