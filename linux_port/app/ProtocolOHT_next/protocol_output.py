# -*- coding: utf-8 -*-
"""Текстовый протокол, простой PDF и PDF из DOCX (Word COM / LibreOffice / docx2pdf)."""
...
def _collect_ttf_in_dir(folder: Path, names: tuple[str, ...]) -> list[Path]:
    found: list[Path] = []
...
def _linux_cyrillic_ttf_candidates() -> list[Path]:
    candidates: list[Path] = []
    seen: set[Path] = set()
    bundle_fonts = application_bundle_dir() / "fonts"
    for folder in [bundle_fonts, *_linux_font_dirs()]:
        for ttf in _collect_ttf_in_dir(folder, _LINUX_FONT_FILENAMES):
            key = ttf.resolve()
            if key not in seen:
                seen.add(key)
...
def cyrillic_ttf_candidates() -> list[Path]:
    """Пути к TTF с кириллицей: Windows Fonts, системные каталоги Linux, bundle/fonts/."""
    if sys.platform == "win32":
        return _windows_cyrillic_ttf_candidates()
    return _linux_cyrillic_ttf_candidates()


def _configure_fpdf_font(pdf: FPDF, content: str) -> None:
    """Latin-1 — Helvetica; кириллица — первый подходящий TTF из ОС или bundle/fonts/."""
    try:
        content.encode("latin-1")
        pdf.set_font("Helvetica", "", 12)
    except UnicodeEncodeError:
        last_err: Exception | None = None
        for i, ttf in enumerate(cyrillic_ttf_candidates()):
            fam = f"AppCyr{i}"
            try:
        ...
        if sys.platform == "win32":
            hint = (
                ...
                "(например arial.ttf или calibri.ttf)."
            )
        else:
            hint = (
                "Для кириллицы в PDF установите шрифты, например:\n"
                "  sudo apt install fonts-dejavu-core fonts-liberation\n"
                "или положите DejaVuSans.ttf в bundle/fonts/ рядом с программой."
            )
        msg = f"{hint} Стандартные шрифты PDF кириллицу не рисуют."
...
def _docx_to_pdf_via_libreoffice(docx_path: Path, pdf_path: Path) -> None:
    lo = _libreoffice_executable()
    if not lo:
        raise RuntimeError(
            "LibreOffice не найден в PATH. Установите, например:\n"
    ...
    proc = subprocess.run(
        [lo, "--headless", "--convert-to", "pdf", "--outdir", str(out_dir), str(docx_path)],
        capture_output=True,
        text=True,
        timeout=180,
        check=False,
    ...
    if not generated.is_file():
        raise RuntimeError(
            f"LibreOffice завершился без ошибки, но файл не найден: {generated}"
        )
    if generated.resolve() != pdf_path.resolve():
        generated.replace(pdf_path)


def _docx_to_pdf_via_docx2pdf(docx_path: Path, pdf_path: Path) -> None:
    try:
        from docx2pdf import convert
    except ImportError as e:
        raise RuntimeError(
            "Для PDF через Microsoft Word установите: pip install docx2pdf\n"
            "и приложение Microsoft Word."
        ) from e
    convert(str(docx_path.resolve()), str(pdf_path.resolve()))


def _docx_to_pdf_non_windows(docx_path: Path, pdf_path: Path) -> None:
    """Сначала LibreOffice (типично для Linux), затем docx2pdf + Word."""
    errors: list[str] = []
    for converter, fn in (
        ("LibreOffice", _docx_to_pdf_via_libreoffice),
        ("docx2pdf", _docx_to_pdf_via_docx2pdf),
    ):
        try:
            fn(docx_path, pdf_path)
            return
        except Exception as e:
            errors.append(f"{converter}: {e}")
    raise RuntimeError(
...
def write_protocol_pdf_from_docx_template(
    ...
    *,
    ...
    """Собирает DOCX из шаблона и конвертирует в PDF (сохраняет оформление)."""
    ...
    try:
        ...
        if sys.platform == "win32":
            ...
            _docx_to_pdf_non_windows(tmp_p, out_p)
    finally:
        try:
            os.unlink(tmp_docx)
        except OSError:
