...
С версии 1.5.2 перезапуск после обновления идёт **напрямую через запуск exe** (отдельный процесс), без `cmd start` — так корректно работают пути с пробелами и символом `!`.
