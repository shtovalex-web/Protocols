...
class TestBuildWindowsExeBundle(unittest.TestCase):
    ...
    def test_write_release_manifest_creates_json_in_out_dir(self):
        mod = _load_build_module()
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            manifest_path = mod._write_release_manifest(
                exe=exe,
                data_dir=data_dir,
                release_dir=root,
            )
            ...
            self.assertEqual(payload["windows"]["relative_path"], "ProtocolOOT.exe")
