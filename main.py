...
────────────────────────────────────────
1. В меню «Администрирование» → «Настройки и данные…» выберите Excel с сотрудниками
   или оставьте файл Data_base.xlsx рядом с программой.
2. Там же выберите шаблон DOCX, если не используется default_protocol.docx.
3. В меню «Администрирование» → «Приказ и комиссия…» заполните номер/дату приказа,
   председателя и членов комиссии, затем сохраните настройки.
4. В главном окне выберите сотрудников, отметьте программы Б / PP / СИЗ / В,
   проверьте дату, номер протокола, оценку и тип проверки.
5. Нажмите «Сформировать протокол», проверьте предварительный просмотр и сохраните
   результат в DOCX или PDF.

Подробная пользовательская инструкция по запуску, Excel-файлу, журналу и частым
проблемам находится в README.md в папке проекта.
